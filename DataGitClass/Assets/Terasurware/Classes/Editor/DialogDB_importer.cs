...
public class DialogDB_importer : AssetPostprocessor {
	private static readonly string filePath = "Assets/Resources/ExcelDB/DialogDB.xlsx";
	private static readonly string exportPath = "Assets/Resources/ExcelDB/DialogDB.asset";
		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(3); p.dialoue = (cell == null ? "" : cell.StringCellValue);
